...
// watches all pages script.js files as well as the pages.js generated by js-includes
gulp.task('pages-js', function(){
	...
	// pages scripts
	var files = [
		config.srcPaths.scripts + '/m-pages.js'
	];
	...
	var tasks = files.map(function(entry, output) {
		return browserify({
			...
			.pipe(source('assets/js/m-pages.js'))
			.pipe(buffer())
...
gulp.task('modules-js', function(){
	...
	// modules scripts for partials output
	var files = [
		config.srcPaths.scripts + '/m-modules.js'
	];

	var tasksMobile = files.map(function(entry, output) {
		return browserify({
			entries: [entry],
			debug: true
			// noParse: [config.bowerDir + '/jquery/dist/jquery.js'] // prevent lag by excluding scripts that dont 'require'
			...
			.pipe(source('assets/js/m-modules.js'))
			.pipe(buffer())
			// Add transformation tasks to the pipeline here.
				.pipe(plugins.sourcemaps.init({loadMaps: true}))
					.pipe(plugins.babel({
